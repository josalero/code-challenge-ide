#!/usr/bin/env python3
"""React challenge runner (Vitest + Testing Library) — JSON on stdin/stdout."""
...
def copy_runner_deps() -> None:
    for name in ("package.json", "vitest.config.ts", "tsconfig.json", "setup.mjs"):
...
def parse_compile_warnings(stderr: str) -> dict:
    """Surface tsc/vite warnings reported on stderr (no separate type-check pass)."""
    messages: list[dict] = []
    pattern = re.compile(r"^(.+\.tsx?):(\d+):\d+ - (?:warning|error)\s+\w+:\s+(.+)$")
